...
export class AdminAuthService {
  ...
  private async checkExistingSession(): Promise<void> {
    ...
    if (session?.user) {
      ...
      let adminUser = await this.fetchAdminUser(session.user.id);

      if (!adminUser && session.user.email) {
        // Try email lookup first
        const { data: adminByEmail, error: emailErr } = await this.supabase
          .from('admin_users')
          .select('*')
          .eq('email', session.user.email)
          .single();

        if (adminByEmail && !emailErr) {
          console.warn('Admin record exists by email but not linked to this auth user id; proceeding with email-linked record');
          adminUser = adminByEmail as AdminUser;
        } else {
          // Auto-create admin record to avoid kicking out legitimate admins
          try {
            const upsertObj = {
              id: session.user.id,
            ...
            const { error: upsertErr } = await this.supabase.from('admin_users').upsert([upsertObj]);

            if (upsertErr) {
              // If RLS policy recursion is detected, log actionable guidance and continue with an
              // in-memory admin user so legitimate admins are not blocked from using the app.
              if ((upsertErr as any)?.code === '42P17') {
                console.error('Failed to auto-create admin user: RLS recursion detected. Run the SQL migration `scripts/migrations/007_relax_rls_for_dev.sql` in the Supabase SQL Editor to fix RLS policies for development. Error:', upsertErr);
              } else {
                console.error('Failed to auto-create admin user during session check', upsertErr);
              }
              adminUser = upsertObj as unknown as AdminUser;
            } else {
              adminUser = await this.fetchAdminUser(session.user.id) || (upsertObj as unknown as AdminUser);
            }
          } catch (e) {
            console.error('Auto-create admin user exception during session check', e);
            adminUser = { id: session.user.id, email: session.user.email, role: 'admin' } as AdminUser;
          }
        }
      }
  ...
  private async performLogin(credentials: LoginCredentials): Promise<LoginResponse> {
    ...
    try {
      ...
      let adminUser = await this.fetchAdminUser(data.user.id);

      // If not found by auth user id, try finding by email and reconcile (fix common setup mismatch)
      if (!adminUser) {
        const { data: adminByEmail, error: emailErr } = await this.supabase
          .from('admin_users')
          .select('*')
        ...
        if (adminByEmail && !emailErr) {
          // Admin record exists but is not linked by auth user id. Use the record found by email.
          console.warn('Admin record found by email but not linked to auth user id; proceeding with email-linked record');
          adminUser = adminByEmail as AdminUser;
        } else {
          // Auto-create an admin_users row so the admin can sign in immediately
          try {
            const upsertObj = {
              id: data.user.id,
              email: credentials.email,
            ...
            if (upsertErr) {
              if ((upsertErr as any)?.code === '42P17') {
                ...
                console.error('Failed to auto-create admin user:', upsertErr);
              }
              // proceed with an in-memory adminUser so login is not blocked
              adminUser = upsertObj as unknown as AdminUser;
            } else {
              adminUser = await this.fetchAdminUser(data.user.id) || (upsertObj as unknown as AdminUser);
            }

            await this.logLoginSuccess(adminUser.id || data.user.id, credentials.email);
          } catch (e: any) {
            console.error('Auto-create admin user exception:', e);
            // proceed but note the DB wasn't updated
            adminUser = {
              id: data.user.id,
              email: credentials.email,
              role: 'admin',
