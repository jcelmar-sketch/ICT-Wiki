...
describe('AdminAuthService', () => {
  ...
  it('reconciles admin record when auth user id differs from admin_users.id (by email match)', (done) => {
    // Build a client where signIn returns a user with a different id than the admin record
    const adminRecord = { ...baseAdmin, id: 'admin-1' };
        ...
        return {
          select: jasmine.createSpy('select').and.returnValue({
            eq: jasmine.createSpy('eq').and.callFake((field: string, value: any) => {
              if (field === 'id') {
                // select by id returns no data initially
                return { single: jasmine.createSpy('single').and.callFake(async () => ({ data: null, error: new Error('Not found') })) };
              }
              if (field === 'email') {
                // select by email returns admin record
                return { single: jasmine.createSpy('single').and.callFake(async () => ({ data: adminRecord, error: null })) };
              }
              return { single: jasmine.createSpy('single').and.callFake(async () => ({ data: null, error: null })) };
            })
          }),
          update: jasmine.createSpy('update').and.callFake(() => ({ eq: jasmine.createSpy('eq').and.resolveTo({ error: null }) }))
        };
      }

      if (table === 'activity_logs') {
        return { insert: jasmine.createSpy('insert').and.resolveTo({ error: null }) };
      }

      return { select: jasmine.createSpy('select').and.resolveTo({ data: [], error: null }) };
    });
    ...
    const client = { auth, from: fromSpy } as any;
    supabaseService.getClient.and.returnValue(client);

    // Re-initialize service to use the new client
    service = TestBed.inject(AdminAuthService);
  ...
  it('auto-creates an admin record when none exists and allows login', (done) => {
    let upsertSpy: jasmine.Spy;

    const fromSpy = jasmine.createSpy('from').and.callFake((table: string) => {
      if (table === 'admin_users') {
        upsertSpy = jasmine.createSpy('upsert').and.resolveTo({ error: null });
        return {
          select: jasmine.createSpy('select').and.returnValue({
            eq: jasmine.createSpy('eq').and.callFake(() => ({ single: jasmine.createSpy('single').and.callFake(async () => ({ data: null, error: new Error('Not found') })) }))
          }),
          upsert: upsertSpy,
        };
      }

      if (table === 'activity_logs') return { insert: jasmine.createSpy('insert').and.resolveTo({ error: null }) };
      return { select: jasmine.createSpy('select').and.resolveTo({ data: [], error: null }) };
    });
    ...
    const auth = {
      signInWithPassword: jasmine.createSpy('signInWithPassword').and.callFake(async ({ email, password }) => {
        if (password === 'correct') {
          return { data: { user: { id: 'auth-abc', email }, session: { access_token: 'access', refresh_token: 'refresh' } }, error: null };
        }
        return { data: { user: null, session: null }, error: new Error('Invalid credentials') };
      }),
      getSession: jasmine.createSpy('getSession').and.resolveTo({ data: { session: null }, error: null }),
