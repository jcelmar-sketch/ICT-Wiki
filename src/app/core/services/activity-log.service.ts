...
export class ActivityLogService {
  ...
  private async performLog(log: Partial<ActivityLog>): Promise<boolean> {
    ...
    if (error) {
      // Detect common RLS recursion error and provide actionable guidance rather than
      // flooding the console with opaque 500s.
      if ((error as any)?.code === '42P17') {
        console.error('Failed to insert activity log due to RLS recursion. Run `scripts/migrations/007_relax_rls_for_dev.sql` in the Supabase SQL Editor. Error:', error);
      } else {
