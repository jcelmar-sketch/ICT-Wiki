...
async function seedData() {
  try {
    // Ensure required tables exist before seeding to avoid partial side effects
    const required = ['admin_users', 'topics', 'articles', 'parts', 'categories'];
    for (const t of required) {
      try {
        const { error: tableErr } = await supabase.from(t).select('id', { head: true });
        if (tableErr) {
          await log(`Required table missing or inaccessible: ${t}`, 'error');
          await log('Please run migrations in the Supabase Dashboard SQL Editor (scripts/migrations) before running this setup.', 'info');
          process.exit(1);
        }
      } catch (e) {
        await log(`Required table missing or inaccessible: ${t} (${e.message})`, 'error');
        await log('Please run migrations in the Supabase Dashboard SQL Editor (scripts/migrations) before running this setup.', 'info');
        process.exit(1);
      }
    }
...
async function main() {
  ...
  try {
    ...
    const m1 = await runMigration('Create Admin Tables', '001_create_admin_tables.sql');
    const m2 = await runMigration('Fix RLS Policies (final)', '006_fix_rls_policies.sql');
    const m3 = await runMigration('Create Audit Triggers', '003_admin_triggers.sql');
    const m4 = await runMigration('Admin fixes & storage (display_order / buckets)', '005_fix_admin_issues.sql');

    // If any migration could not be executed automatically, try verifying that required tables already exist
    if (!m1 || !m2 || !m3 || !m4) {
      await log('\nOne or more migrations could not be applied automatically via the JS SDK. Running verification checks...', 'warn');

      // Verify required tables were created by a manual migration run
      const requiredTables = ['admin_users','categories','articles','parts','topics'];
      const missing = [];
      ...
      if (missing.length === 0) {
        await log('Required tables exist; assuming migrations were applied manually. Proceeding to seeding.', 'success');
      } else {
        await log('\nRequired tables missing: ' + missing.join(', '), 'error');
        await log('Please run the following files manually via the Supabase Dashboard SQL Editor:', 'info');
        console.log('  - scripts/migrations/001_create_admin_tables.sql');
        console.log('  - scripts/migrations/006_fix_rls_policies.sql');
        console.log('  - scripts/migrations/003_admin_triggers.sql');
        console.log('  - scripts/migrations/005_fix_admin_issues.sql');
        console.log('\nAfter applying migrations, re-run this script: `npm run db:setup`\n');
        process.exit(1);
      }
    }
