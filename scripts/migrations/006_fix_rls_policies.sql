...
-- Allow inserts from authenticated admins (standardize name)
...
DROP POLICY IF EXISTS "Admins can read articles" ON storage.objects;
CREATE POLICY "Admins can read articles" ON storage.objects
  FOR SELECT
  TO public
  USING (bucket_id = 'articles');

DROP POLICY IF EXISTS "Admins can delete articles" ON storage.objects;
CREATE POLICY "Admins can delete articles" ON storage.objects
  FOR DELETE
  TO authenticated
  USING (
    bucket_id = 'articles' AND
...
DROP POLICY IF EXISTS "Admins can upload parts" ON storage.objects;
CREATE POLICY "Admins can upload parts" ON storage.objects
  FOR INSERT
  TO authenticated
  WITH CHECK (
    bucket_id = 'parts' AND
...
DROP POLICY IF EXISTS "Public can read parts" ON storage.objects;
CREATE POLICY "Public can read parts" ON storage.objects
  FOR SELECT
  TO public
